...
function Accept ( props )
{
    ...
    const AcceptRequests = () =>
    {
        ...
        fetch(api_url + "admin/approve_signup", requestOptions )
    ...
    return (
        ...
        >
            <div className="relative p-4 w-full mx-auto custom-width5 mt-[3vw]">
                <div className="relative bg-white rounded-md shadow custom-height9">
                    ...
                    <form
                        className="px-[1.8vw] py-[1.2vw] sm:pb-8"
                        onSubmit={ ( e ) =>
                        {
                            e.preventDefault();
                            AcceptRequests();
                    ...
                    >
                        <h1 className="text-3xl sm:text-2xl">Select user access rights:</h1>
                        <br />
                        <div class="flex items-center mb-4">
                            <input id="data-checkbox" type="checkbox" checked={ data } onChange={ handleData } class="w-6 h-6 text-secondary bg-white text-2xl border-gray-300 rounded focus:ring-0" />
                            <label for="data-checkbox" class="ms-2 sm:text-[1.4rem] lm:text-[1.6rem] tab:text-[1.8rem] sl:text-[1.8vw] lt:text-[1.2vw]">Data Download</label>
                        </div>
                        <div class="flex items-center mb-4">
                            <input id="report-checkbox" type="checkbox" checked={ report } onChange={ handleReport } class="w-6 h-6 text-secondary bg-white text-2xl border-gray-300 rounded focus:ring-0" />
                            <label for="report-checkbox" class="ms-2 sm:text-[1.4rem] lm:text-[1.6rem] tab:text-[1.8rem] sl:text-[1.8vw] lt:text-[1.2vw]">Reports Download</label>
                        </div>
                        <br />
                        { error && (
