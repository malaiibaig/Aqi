...
function Chart ( props )
{
	...
	const Default = () =>
	{
		...
		fetch( home, requestOptions )
			...
			.then( ( result ) =>
			{
				if ( result )
				{
					setStationId( result.default_aqi.station );
				}
			} )
	...
	const StationData = () =>
	{
		...
		{
			const formdata = new FormData();
			formdata.append( "station_id", stationId2 );
	...
	const GasesData = async () =>
	{
		...
		if ( stationId )
		{
			...
			fetch( api_url + `gases-new?station=${ stationId }`, requestOptions )
				.then( ( response ) => response.json() )
				.then( ( result ) =>
				{
					if ( result )
		...
		if ( stationId2 )
		{
			...
			fetch( api_url + `gases-new?station=${ stationId2 }`, requestOptions )
				.then( ( response ) => response.json() )
				.then( ( result ) =>
				{
					if ( result )
	...
	const PMData = async () =>
	{
		...
		fetch( api_url + `gases-new?station=${ stationId }`, requestOptions )
			...
			.then( ( result ) =>
			{
				if ( result )
				{
					const list = Object.keys( result ).filter( ( key ) => key.startsWith( "PM" ) );
					setGasesList( list );
					setUnits( result.units );
				}
			} )
